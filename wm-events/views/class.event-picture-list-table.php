...
class WM_Event_Picture_List_Table extends WP_List_Table {
    ...
    function prepare_items() {
        ...
        $this->total_items = WM_Event_Picture_Model::count_all( $eid );
        $this->total_found_items = $this->total_items;

        $items = WM_Event_Picture_Model::find_by_event( $eid, array (
            'per_page' => $per_page,
            'paged' => $current_page
        ) );
        ...
        foreach ( $items as $item ) {
            $this->found_items[] = array(
                'ID' => $item->event_picture_id,
                'picture' => $item->event_picture_filename,
                'filename' => $item->event_picture_filename,
                'description' => $item->event_picture_description,
                'uploaded' => $item->event_picture_uploaded
            );
        }
    ...
    function column_default( $item, $column_name ) {
        ...
        switch ( $column_name ) {
            ...
            case 'picture':
                ...
                return '<img src="' . WM_EVENT_UPLOAD_URL . $eid . '/' . $file_info['filename'] . '_thumb.' . $extension . '" alt="" title="" height="75">';
            case 'filename':
    ...
    function column_name( $item ) {
        ...
        $_wpnonce = wp_create_nonce( 'event_picture_delete_' . $item['ID'] );
        $actions = array(
            'delete' => sprintf(
                    '<a href="'.admin_url( 'admin.php' ).'?action=%s&eid=%d&epid=%d&_wpnonce=%s">Excluir imagem</a>',
                    $wm_event_config['slug_action_delete_picture'],
        ...
        ?>
            <div class="tablenav <?php echo esc_attr( $which ); ?>">
                <div class="alignleft actions">
                    <button class="button" type="button" onclick="location.href='<?php echo admin_url( 'admin.php' ) . '?page=' . $wm_event_config['slug_page_upload_view'] . '&eid=' . $eid; ?>'">Carregar imagem</button>
                </div>
                <?php
    ...
    function views() {
        ?><ul class="subsubsub">
            <li class="all"><a href="#"><?php echo __( 'All' ) ?> <span class="count">(<?php echo $this->event_count ?>)</span></a></li>
        </ul><?php
    }
