...
<?php wp_enqueue_style( 'w-obra', W_OBRA_PLUGIN_URL . 'assets/css/w-obra.css' ); ?>

<?php if ( empty( $obra->obra_id ) ) : ?>

    <script>location.href="<?php menu_page_url( $W_OBRA_config['slug_page_list_view'] ); ?>";</script>

<?php elseif ( $upload_size_unit == 0 ) : ?>
    
    <div class="wrap osb-obra-upload">
        <h2>Obra - <?php echo $obra->obra_name ?></h2>
...
<?php else : ?>
    ...
    <div class="wrap osb-obra-upload">
        ...
        <form enctype="multipart/form-data" id="fileupload" method="post" action="#" class="media-upload-form type-form validate">
            ...
            <div class="submit">
                <button type="button" class="button-primary" onclick="obraUploadView.close();">Concluír</button>
            </div>
            <div id="console"></div>
        </form>
    </div>

    <script type="text/javascript" src="<?php echo W_OBRA_PLUGIN_URL . 'assets/js/plupload.full.min.js'; ?>"></script>
    <script type="text/javascript" src="<?php echo W_OBRA_PLUGIN_URL . 'assets/js/i18n/pt_BR.js'; ?>"></script>
    <script type="text/javascript">
        ...
        var obraUploadView = {
            ...
            init : function() {
                ...
                var obraUploader = new plupload.Uploader({
                    runtimes: 'html5,flash,silverlight,html4',
                    browse_button: 'btn-select-files',
                    drop_element: 'drag-drop-area',
                    container: document.getElementById('plupload-upload-ui'),
                    url: '<?php echo W_OBRA_PLUGIN_URL . 'w-obra-do_upload.php?eid=' . $obra->obra_id; ?>',
                    flash_swf_url: '<?php echo W_OBRA_PLUGIN_URL . 'assets/js/Moxie.swf' ?>',
                    silverlight_xap_url: '<?php echo W_OBRA_PLUGIN_URL . 'assets/js/Moxie.xap' ?>',
                    filters: {
                    ...
                    init: {
                        PostInit: function() {
                            ...
                            if (obraUploader.features.dragdrop) {

                                jQuery("#drag-drop-area")

                                    .on("dragenter", function(obra){
                                        obra.dataTransfer.dropEffect = "copy";
                        ...
                        FileUploaded: function(up, file, info) {
                            ...
                            if (response.result == null) {
                                jQuery("#"+file.id).html(
                                    '<img class="pinkynail" src="<?php echo W_OBRA_UPLOAD_URL . $obra->obra_id . '/'; ?>' + response.thumbnail + '" alt="">' +
                        ...
                        UploadComplete: function(up, files) {
                            obraUploadView.uploading = false;
                        },
            ...
            close : function() {
                if (( ! obraUploadView.uploading) || (obraUploadView.uploading && confirm("Ainda não terminamos de carregar sua lista de arquivos, deseja concluír assim mesmo?"))) {
                    location.href = '<?php echo menu_page_url( $W_OBRA_config['slug_page_form_view'], false ) . '&eid=' . $obra->obra_id ; ?>';
                }
            }

        };
        jQuery( function() { obraUploadView.init(); });
